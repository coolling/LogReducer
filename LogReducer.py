...
lock = threading.RLock()#用于同步的线程锁
gl_threadTotTime = 0#用于累积线程执行时间的全局变量
gl_errorNum = 0#用于累积错误数量的全局变量
...
def add_argument(parse):
    ...
    parse.add_argument("--Template", "-T", default="no", help="The template directory inclues needed templates(each input file corresponds to a directory(such as LogA/ LogB/ ..)")
    ...
    #添加结构化日志选择的命令行参数
    parse.add_argument("--Structured", "-S", default="N", choices=["N", "Y"], help="is Structured Logs?")
    
    #添加命令行参数供用户选择
    parse.add_argument("--ZipType", "-Z", default="LZMA", choices=["LZMA", "gzip"], help="zip type")
...
def check_args(args):
    ...
    if (not os.path.exists(args.Input)):
        ...
        return 0
    #如果是结构化日志 不需要模板
    if (args.Structured=="N" and not os.path.exists(args.Template)):
...
#文件名 开始处理的.col 最后处理的.col segment文件夹路径 ./out/ ./out/tmp /template
def procFiles(typename, fileBeginNo, fileEndNo, now_input, now_output, now_temp, type_template,zip_type,struct):
    t1 = time.time()
    #parser
    ...
    for i in range(fileBeginNo, fileEndNo + 1):
        ...
        if zip_type=="LZMA":
            ...
            zip_path = str(now_output) + str(filename) + ".zip"
            compression_order = "7za a -tzip " + zip_path + " " + now_temp + filename + "/*" 
...
#文件名 所有拆分的.col文件 segment文件夹路径 ./out/ ./out/tmp /template
def threadsToExecTasks(typename, files, now_input, now_output, now_temp, type_template,zip_type,struct):
...
#文件名 开始处理的.col 最后处理的.col segment文件夹路径 ./out/ ./out/tmp /template
        future = threadPool.submit(procFiles, typename, curFileNumBegin, curFileNumEnd, now_input, now_output, now_temp, type_template,zip_type,struct)
...
if __name__ == "__main__":
    ...
    mode = args.Mode
    zip_type=args.ZipType
    struct=args.Structured
    ...
    if (not os.path.exists(now_output)):
        ...
        #文件名 所有拆分的.col文件 segment文件夹路径 ./out/ ./out/tmp /template
    threadsToExecTasks(now_type, all_files, now_input, now_output, now_temp, type_template,zip_type,struct)
